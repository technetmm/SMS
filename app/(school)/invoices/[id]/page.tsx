...
import { getInvoiceById } from "@/app/(school)/invoices/actions";
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            ...
            <Button asChild>
              <Link href={`/invoices/${invoice.id}/pdf`}>Download Invoice</Link>
            </Button>
          </div>
        }
      />
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader><CardTitle>Original Fee</CardTitle></CardHeader>
          <CardContent>${Number(invoice.originalAmount).toFixed(2)}</CardContent>
        </Card>
        <Card>
        ...
        <Card>
          <CardHeader><CardTitle>Final Fee</CardTitle></CardHeader>
          <CardContent>${Number(invoice.finalAmount).toFixed(2)}</CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle>Paid / Remaining</CardTitle></CardHeader>
          <CardContent>
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          <p><strong>Section:</strong> {invoice.enrollment.section.class.name} • {invoice.enrollment.section.name}</p>
          <p>
        ...
        <CardContent className="space-y-4">
          {invoice.payments.map((payment) => (
            <div key={payment.id} className="rounded-md border p-3 text-sm">
              <p>
                <strong>{payment.method}</strong> • ${Number(payment.amount).toFixed(2)} • {new Intl.DateTimeFormat("en-US", { dateStyle: "medium", timeStyle: "short" }).format(payment.createdAt)}
              </p>
              {payment.refunds.length ? (
                <div className="mt-2 space-y-1 text-xs text-muted-foreground">
                  {payment.refunds.map((refund) => (
                    <p key={refund.id}>
                      Refund: ${Number(refund.amount).toFixed(2)} • {refund.reason ?? "No reason"}
                    </p>
                  ))}
                </div>
              ) : null}
